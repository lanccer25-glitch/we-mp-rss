...
Do not commit `config.yaml`, tokens, cookies, or data from `data/`. Start from `config.example.yaml`, keep secrets in local config only, and review `SECURITY.md` before changing auth, webhooks, or access-key flows.
